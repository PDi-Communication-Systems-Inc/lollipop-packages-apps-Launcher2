...
public final class Launcher extends Activity
    ...
    private static final String BOOTING_AFTER_WIPE_OR_FACTORY_RESET= "booting_after_wipe_or_factory_reset";
        ...
        //constants for restoring wifi
    private static final String WIFI_BACKUP_RELATIVE_PATH= "/admin";
    private static final String WIFI_BACKUP_FILE= "wifi.bk";
    private static final String WIFI_RESTORE_PATH= "/data/misc/wifi";
    private static final String WIFI_RESTORE_FILE= "wpa_supplicant.conf";
    private static final String PACKAGE_RESTORE_DATA_TO_SYSTEM = "com.pdiarm.restoredatatosystem";
    private static final String COMPONENT_RESTORE_DATA_TO_SYSTEM= "com.pdiarm.restoredatatosystem.WriteDataToSystem";
    private static final int REQUEST_RESTORE_WIFI = 32;    
    boolean m_pendingWifiRestoration = false;
    private Object m_wifi_bk_file_Lock = new Object();
    private final int NUMBER_OF_WIFI_RETRIES = 5;
    private final int MILLISEC_BETWEEN_WIFI_RETRIES = 1000;

     //One time ask for wifi connection to be set
    private static boolean mHasWifiCofigurationBeenAsked = false;
   ...
   //   one time wifi enable per reboot
    ...
    //Acquire wakelock to prevent the device from going to sleep
    ...
    private void releasePartialWakeLock() {
        ...
        if (partialWakeLock != null) {
            ...
            // should never happen during normal workflow
            Log.e(TAG, "PartialWakelock reference is null");
        }
    }
    ...
    private void CopyLauncherDataForNewUser()	{
		
    	String dataDir = getDataDir();
		try {
			File fs = new File("/data/data/com.android.launcher/databases/launcher.db"); //admin user
			File fd = new File(dataDir + "/databases/launcher.db"); //non admin user
			if(fs.getAbsolutePath().equalsIgnoreCase(fd.getAbsolutePath())) {
				Log.i(TAG, "Admin logged in for the first time ");
			}
			else {
				//non admin user
				Utils.copyDirectoryOneLocationToAnotherLocationNoAdmin(fs, fd) ;
			}
		} 
    ...
    private void CopySDcardDataFromAdmin(){
    	
    	String dataDir = getDataDir();
    	
    	try {
			File fs = new File("/data/data/com.android.launcher/databases/launcher.db");
			File fd = new File(dataDir); 
			//File fd = new File("/data/user/" + newUserIdString + "/com.android.launcher/databases/launcher.db"  );
			Utils.copyDirectoryOneLocationToAnotherLocationNoAdmin(fs, fd) ; 			
		} 
        catch (FileNotFoundException e) {
        	if ((e != null) && (e.getMessage() != null))
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fullWakeLock = powerManager.newWakeLock((PowerManager.SCREEN_BRIGHT_WAKE_LOCK | PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP), "Launcher - FULL WAKE LOCK");
        ...
        DismissCling(Cling.FOLDER_CLING_DISMISSED_KEY);
        
        //create the database dir and share it
        String dataDir = getDataDir();
        File localDirPath = new File(dataDir + "/databases/");
    	if(!localDirPath.isDirectory())
				localDirPath.mkdirs();
    	localDirPath.setReadable(true, false);
    	localDirPath.setWritable(true, false);
    	
    	SharedPreferences prefs = getSharedPreferences(PDI_PREFERENCES, MODE_PRIVATE);
    	Boolean l_IsBootingAfterWipeOrFactoryReset = prefs.getBoolean(BOOTING_AFTER_WIPE_OR_FACTORY_RESET, true);
		if(l_IsBootingAfterWipeOrFactoryReset)	{
			CopyLauncherDataForNewUser();	
		}
    	
        //End - Do not show clings
...
private void setThirdPartyLauncher(Context context) {
   ...
   ComponentName cN = new ComponentName("pdiarm.com.pditv", "pdiarm.com.pditv.PDiMainActivity");
   ...
   ComponentName newPreferredActivity = new ComponentName("pdiarm.com.pditv", "pdiarm.com.pditv.PDiMainActivity");
   ...
   launchIntent.setClassName("pdiarm.com.pditv", "pdiarm.com.pditv.PDiMainActivity");
    ...
    protected void onActivityResult(
        ...
        {	
        	...
        	{ 
        		...
        		Bundle extras = data.getExtras();
        		if(extras !=null) {
        			param_file = extras.getString("FILE");//NOTE: getString returns NULL		    
        		}
        		...
        		{  
		    		...
		    		if (!isConnectedOrConnectingToAny(getApplicationContext()))
		    		{
			    		...
			    		{
			    			Log.e(TAG,"wifiManager.setWifiEnabled(true) returned false - could not enable wifi");			    			
			    		}		    					    		
        			}
	        		else
	        		{
        			...
        			Log.e(TAG, "REQUEST_RESTORE_WIFI returned RESULT_OK but extras not set ");
        	} 
        	else
        		Log.e(TAG, "Result code is not OK for request restore wifi");
	...
	//		l_isdeviceMasterOrSlaveConfiguredCalled = RestoreMasterOrSlaveConfiguration();		
        ...
        if (isWidgetDrop) {
          //  final int appWidgetId;
            int appWidgetId = data != null ? data.getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, -1)
                    : -1;
            if (appWidgetId < 0) {
 ...
 // Process any items that were added while Launcher was away
    ...
    protected void onPause() {
        ...
        if(!localDirPath.isDirectory())
				localDirPath.mkdirs();
		File dbfile = new File( dataDir + "/databases/" , "launcher.db");
